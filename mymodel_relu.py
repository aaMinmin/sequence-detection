...
with open(os.path.join('./result/bilstm_attention/', 'Test_seq_relu.csv'), 'w', newline='') as f4:  # 实验结果保存的地方
    writer = csv.writer(f4)
    for z in testseq:
        writer.writerow([z])
#
with open(os.path.join('./result/bilstm_attention/', 'Truth_score_relu.csv'), 'w', newline='') as f6:  # 实验结果保存的地方
...
output = Dense(units, activation='relu')(attn_output)
...
checkpoint_filepath = './model/bilstm_attention/model_epoch.285_valloss.4.6585_valR2.0.9148.hdf5'
checkpoint_filepath_many = os.path.join('./model/bilstm_attention/',
...
with open(os.path.join('./result/bilstm_attention/', 'prediction.csv'), 'w', newline='') as f5:
