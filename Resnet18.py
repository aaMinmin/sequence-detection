...
with open(os.path.join('./result/Resnet18/', 'testseq__resnet2.csv'), 'w', newline='') as f4:  # 实验结果保存的地方
    writer = csv.writer(f4)
    for z in testseq:
        writer.writerow([z])
#
with open(os.path.join('./result/Resnet18/', 'truth__resnet2.csv'), 'w', newline='') as f6:  # 实验结果保存的地方
...
checkpoint_filepath = './model/resnet/model_epoch.023_valloss.9.3704_valR2.0.7710.hdf5'
checkpoint_filepath_many = os.path.join('./model/resnet/',
...
with open(os.path.join('./result/Resnet18/', 'pred_score__resnet2.csv'), 'w', newline='') as f5:
