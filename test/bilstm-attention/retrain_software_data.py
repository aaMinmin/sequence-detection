...
with open(os.path.join('./', 'Test_seq_relu.csv'), 'w', newline='') as f4:  # 实验结果保存的地方
    writer = csv.writer(f4)
    for z in testseq:
        writer.writerow([z])
#
with open(os.path.join('./', 'Truth_score_relu.csv'), 'w', newline='') as f6:  # 实验结果保存的地方
...
# output = Dense(units, activation='relu')(output)
# output = Dropout(0.2)(output)
...
checkpoint_filepath = './model_epoch.285_valloss.4.6585_valR2.0.9148.hdf5'
checkpoint_filepath_many = os.path.join('./',
...
if os.path.exists(checkpoint_filepath):
    ...
    #                metrics=[r2]
    #                )  # Adam
    # model1.fit(seqs_train, scores_train,epochs=200, callbacks=[model_checkpoint_callback], verbose=2,
...
with open(os.path.join('./', 'prediction.csv'), 'w', newline='') as f5:
