...
with open(os.path.join('./', 'testseq_lstm.csv'), 'w', newline='') as f4:  # 实验结果保存的地方
    writer = csv.writer(f4)
    for z in testseq:
        writer.writerow([z])
#
with open(os.path.join('./', 'truth__lstm.csv'), 'w', newline='') as f6:  # 实验结果保存的地方
...
output1 = Dense(units,activation='relu')(lstm_out)
...
checkpoint_filepath = './model_epoch.274_valloss.8.1501_valR2.0.8118.hdf5'
checkpoint_filepath_many = os.path.join('./model_lstm/',
...
if os.path.exists(checkpoint_filepath):
    ...
    print("R_2：")
    print(R_2)
    MRE = K.sum(K.abs(cl-scores_test)/scores_test)
    print("MRE：")
    print(MRE)
# print(cl)
# qall = q.history['loss']
# qaall = q.history['val_loss']
    ...
    plt.show()
    plt.savefig('./model_loss_lstm.jpg')

    R_2 = r2_score(scores_test[:len(cl)], cl)
    print("R_2：")
    print(R_2)

with open(os.path.join('./', 'pred_score_lstm.csv'), 'w', newline='') as f5:
