...
with open(os.path.join('./result/lstm/', 'testseq_lstm.csv'), 'w', newline='') as f4:  # 实验结果保存的地方
    writer = csv.writer(f4)
    for z in testseq:
        writer.writerow([z])
#
with open(os.path.join('./result/lstm/', 'truth__lstm.csv'), 'w', newline='') as f6:  # 实验结果保存的地方
...
checkpoint_filepath = './model/lstm/model_epoch.274_valloss.8.1501_valR2.0.8118.hdf5'
checkpoint_filepath_many = os.path.join('./model/lstm/',
...
with open(os.path.join('./result/lstm/', 'pred_score_lstm.csv'), 'w', newline='') as f5:
