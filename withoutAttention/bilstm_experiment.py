...
from tensorflow.keras.layers import Input, Dense, LSTM, Embedding, Dropout, Masking, BatchNormalization, Flatten, \
    Softmax, Bidirectional, Activation, Permute,LayerNormalization
import pickle
...
with open(os.path.join('../', 'mysavedata/ATCG/Train_data.pkl'), 'rb+') as f2:
    seqs_train1 = pickle.load(f2)
with open(os.path.join('../', 'mysavedata/ATCG/Train_data2.pkl'), 'rb+') as f2:
    seqs_train2 = pickle.load(f2)
seqs_train = np.hstack((seqs_train1, seqs_train2))

with open(os.path.join('../', 'mysavedata/ATCG/Train_score.pkl'), 'rb+') as f:
    scores_train1 = pickle.load(f)
scores_train1 = [np.float32(x) for x in scores_train1]
scores_train1 = np.array(scores_train1)
with open(os.path.join('../', 'mysavedata/ATCG/Train_score2.pkl'), 'rb+') as f:
...
with open(os.path.join('../', 'mysavedata/ATCGtest/Test_data.pkl'), 'rb+') as f4:
    seqs_test1 = pickle.load(f4)
with open(os.path.join('../', 'mysavedata/ATCGtest/Test_data2.pkl'), 'rb+') as f4:
    seqs_test2 = pickle.load(f4)
seqs_test = np.hstack((seqs_test1, seqs_test2))

with open(os.path.join('../', 'mysavedata/ATCGtest/Test_score.pkl'), 'rb+') as f3:
    scores_test1 = pickle.load(f3)
scores_test1 = [np.float32(x) for x in scores_test1]
scores_test1 = np.array(scores_test1)
with open(os.path.join('../', 'mysavedata/ATCGtest/Test_score2.pkl'), 'rb+') as f3:
...
with open(os.path.join('../', 'mysavedata/ATCGtest/Test_seq.pkl'), 'rb+') as f5:
    testseq1 = pickle.load(f5)
with open(os.path.join('../', 'mysavedata/ATCGtest/Test_seq2.pkl'), 'rb+') as f5:
    testseq2 = pickle.load(f5)
testseq = np.hstack((testseq1, testseq2))

with open(os.path.join('./result/ATCGresult/', 'Test_seq.csv'), 'w', newline='') as f4:  # 实验结果保存的地方
    writer = csv.writer(f4)
    for z in testseq:
        writer.writerow([z])
#
with open(os.path.join('./result/ATCGresult/', 'Truth_score.csv'), 'w', newline='') as f6:  # 实验结果保存的地方
...
# attn_output, alpha_n = attention(lstm_out,units*2)
# lstm_out = Flatten()(lstm_out)
# lstm_out = LSTM(units, use_bias=True, recurrent_activation='sigmoid', kernel_initializer='glorot_uniform'
#                               )(lstm_out)
output = Dense(units,activation='relu')(lstm_out)
...
def loss_(weight):
    def loss_function(y_true, y_pred):
        ...
        mse = tf.losses.mse(y_true, y_pred)*abs_true
...
checkpoint_filepath = './model4/model_epoch.262_valloss.8.1655_valR2.0.8117.hdf5'
checkpoint_filepath_many = os.path.join('./model4/',
...
if os.path.exists(checkpoint_filepath):
    ...
    # #                metrics=[r2]
    # #                )  # Adam
    # model1.fit(seqs_train, scores_train,epochs=20, callbacks=[model_checkpoint_callback], verbose=2,
    #                batch_size=batchsz, validation_split=0.2)
    cl = model1.predict(seqs_test, batch_size=1)
    R_2 = r2_score(scores_test, cl)
    print("R_2：")
    print(R_2)
    ...
    # model1.save('./my_model64_loss.h5')
    #
    # num = 1
    ...
    # plt.show()
    # plt.savefig('./model_loss_64loss.jpg')

    # R_2 = r2_score(scores_test[:len(cl)], cl)
    # print("R_2：")
    # print(R_2)

with open(os.path.join('./result/', 'prediction8k.csv'), 'w', newline='') as f5:
